...
def extract_sng(caption):
    ...
    relations = [
        {
            "subject": entities[x["subject"]],
            "object": entities[x["object"]],
            "relation": x["relation"],
        }
...
def preprocess_precomputed_base(sample, spacy_nlp, keywords_list, tokenizer):
        ...
        if keyword != "" and keyword in caption:
            ...
            if len(tmp_keywords) > 0:
                selected_keywords = random.sample(
                    tmp_keywords, k=min(int(len(tmp_keywords) * 1.0), 1)
                )
    ...
    replaced_token_dict = tokenizer(
        text=replaced_caption,
        return_tensors="pt",
        padding="max_length",
        truncation=True,
    )
    replaced_tokens, replaced_attention_mask = (
        replaced_token_dict["input_ids"][0],
        replaced_token_dict["attention_mask"][0],
    )

    replaced_tokens = torch.where(
        replaced_tokens == 49408,
...
class CaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for keyword in keywords:
            if keyword != "" and keyword in caption:
                ...
                if len(tmp_keywords) > 0:
                    ...
                    for selected_keyword in selected_keywords:
                        replaced_caption = replaced_caption.replace(
                            selected_keyword, "[$]"
                        )
                else:
                    replaced_caption = f"a photo of [$] that {caption}"
        ...
        tokens, attention_mask = (
            ...
            token_dict["attention_mask"][0],
        )

        replaced_token_dict = self.tokenizer(
            text=replaced_caption,
            return_tensors="pt",
            padding="max_length",
            truncation=True,
...
# TODO: Load cc3m_vi instead https://github.com/dinhanhx/velvet
def build_loader(args, tokenizer, accelerator):
    data_names = {
        "dataset1": "dangne/gcc_caption_only",
...
class FashionIQDataset(Dataset):
    ...
    def __init__(
        ...
        for dress_type in dress_types:
            with open(
                dataset_path / "captions" / f"cap.{dress_type}.{split}.json"
            ) as f:
            ...
            if self.mode == "relative":
                ...
                if self.split in ["train", "val"]:
                    reference_image_path = (
                        self.dataset_path / "images" / f"{reference_name}.jpg"
                    )
                    reference_image = self.preprocess(
                        PIL.Image.open(reference_image_path), return_tensors="pt"
                    )["pixel_values"][0]
                    target_name = self.triplets[index]["target"]
                    target_image_path = (
                        self.dataset_path / "images" / f"{target_name}.jpg"
                    )
                    target_image = self.preprocess(
                        PIL.Image.open(target_image_path), return_tensors="pt"
                    )["pixel_values"][0]
                ...
                image = self.preprocess(
                    PIL.Image.open(image_path), return_tensors="pt"
                )["pixel_values"][0]
        ...
        try:
            if self.mode == "relative":
                ...
                if self.split in ["train", "val"]:
                    ...
                    target_image_path = (
                        self.dataset_path / self.name_to_relpath[target_hard_name]
                    )
                    target_image = self.preprocess(
                        PIL.Image.open(target_image_path), return_tensors="pt"
                    )["pixel_values"][0]
...
class CIRCODataset(Dataset):
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == "relative":
            ...
            reference_img_path = self.img_paths[
                self.img_ids_indexes_map[reference_img_id]
            ]
            reference_img = self.preprocess(
                PIL.Image.open(reference_img_path), return_tensors="pt"
            )["pixel_values"][0]
            ...
            if self.split == "val":
                ...
                target_img_path = self.img_paths[
                    self.img_ids_indexes_map[target_img_id]
                ]
                target_img = self.preprocess(
                    PIL.Image.open(target_img_path), return_tensors="pt"
                )["pixel_values"][0]
            ...
            return {"image": img, "image_name": img_id}
